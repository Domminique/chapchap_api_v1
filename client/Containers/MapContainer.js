...
export const MapContainer = ({ route, navigation }) => {
  ...
  const currentMapCamera = useRef({
    ...
    longitude: 36.74296376960316, // Empire State Building Coordinates
  ...
  return (
    <View style={tw`flex flex-col justify-end items-center w-full h-full`}>
      ...
      <AnimatedFAB
        icon={"lightning-bolt"}
        label={"Start logistic planning"}
        extended={true}
        onPress={() => navigation.navigate("Survey")}
        visible={true}
        animateFrom={"right"}
        iconMode={"static"}
        style={tw`bottom-78 pl-2 right-4 absolute rounded-4 bg-green-600`}
      />
